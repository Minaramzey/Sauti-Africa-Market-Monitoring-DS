...
from code.forecast_models import ForecastModels, ModelConfig
...
def walk_forward_validation(data, n_test, cfg):
	...
	for i in range(len(test)):
		...
		yhat = fm.exp_smoothing_onestep(history, cfg)
...
def score_model(data, n_test, cfg, debug=False):
	...
	if debug:
		...
		result, _ = walk_forward_validation(data, n_test, cfg)
	else:		
		try:
...
if __name__ == '__main__':
	...
	mc = ModelConfig()
	cfg_list = mc.simple_configs(max_length=max_length, seasonal=[1])
